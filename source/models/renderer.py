...
from source.models.idt_nerf import BaldNeRF, FrequencyEmbedder
...
class Graph(torch.nn.Module):
    ...
    @torch.no_grad()
    def squeeze_nerf(self, data_dict):
        if not hasattr(self, 'squeezed_nerf'):
            self.squeezed_nerf = None

        if self.squeezed_nerf is None:
            # Squeeze the NeRF into Depthmap
            dummy_iteration = 10000
            B, _, H, W = data_dict.image.shape
            output_dict = self.render_image_at_specific_rays \
                (self.opt, data_dict, iter=dummy_iteration, mode="val")

            self.squeezed_nerf = einops.rearrange(output_dict.depth, "B (H W) C -> B H W C", H=H, W=W)
            self.squeezed_nerf = self.squeezed_nerf.squeeze(-1)
            return self.squeezed_nerf
        else:
            return self.squeezed_nerf

    @torch.no_grad()
    def unsqueeze_nerf(self):
        self.squeezed_nerf = None

    def define_renderer(self, opt: Dict[str, Any]):

        if opt.arch.layers_feat is None:
            self.nerf = BaldNeRF(opt).to(self.device)
        else:
    ...
    def forward(self, opt: Dict[str, Any], data_dict: Dict[str, Any], iter: int,
        ...
        if img_idx is not None:
            ...
            if opt.nerf.rand_rays and mode in ["train", "test-optim"]:
                ...
                ray_idx = torch.randperm(H * W, device=self.device)[
                          :opt.nerf.rand_rays // nbr_img]  # the same rays are rendered for all the images here
    ...
    def render_image_at_specific_pose_and_rays(self, opt: Dict[str, Any], data_dict: Dict[str, Any], depth_range: Any,
                                               pose: torch.Tensor,
        ...
        if ray_idx is None and pixels is None:
            ...
            ret = self.render(
                opt, pose, intr=intr, mode=mode, H=H, W=W, depth_range=depth_range, iter=iter
            )  # [B,HW,3],[B,HW,1]
        else:
            # render only the ray_idx or pixels specified. If too large, use per_slice here.
            ret = self.render(
                opt, pose, intr=intr, pixels=pixels, ray_idx=ray_idx, mode=mode, H=H, W=W, depth_range=depth_range,
                iter=iter
            )  # [B,N,3],[B,N,1]
            ret.ray_idx = ray_idx
    ...
    def render_image_at_specific_rays(self, opt: Dict[str, Any], data_dict: Dict[str, Any], iter: int,
                                      ...
                                      ray_idx: torch.Tensor = None, mode: str = 'train', per_slice: str = False) -> \
    Dict[str, Any]:
        ...
        if opt.nerf.depth.param == 'inverse':
            depth_range = opt.nerf.depth.range
        elif opt.nerf.depth.param == 'monoguided':
            depth_range = data_dict['depth_est']
        elif self.opt.nerf.depth.param == 'datainverse':
        ...
        if img_idx is not None:
            if isinstance(img_idx, (tuple, list)):
                ...
                if isinstance(depth_range, torch.Tensor):
                    depth_range = depth_range[img_idx]
            else:
                pose = pose[img_idx].unsqueeze(0)  # (1, 3, 4)
                intr = intr[img_idx].unsqueeze(0)  # (1, 3, 3)
                if isinstance(depth_range, torch.Tensor):
                    depth_range = depth_range[img_idx].unsqueeze(0)
                img_idx = [img_idx]  # make it a list of a single element
        H, W = data_dict.image.shape[-2:]

        if ray_idx is None and pixels is None:
            # rendere the full image
            ret = self.render_by_slices(opt, pose, intr=intr, mode=mode, H=H, W=W, depth_range=depth_range,
                                        iter=iter) if (opt.nerf.rand_rays and per_slice) else \
                self.render(opt, pose, intr=intr, mode=mode, H=H, W=W, depth_range=depth_range,
                            iter=iter)  # [B,HW,3],[B,HW,1]
        else:
    ...
    def render(self, opt: Dict[str, Any], pose: torch.Tensor, H: int, W: int, intr: torch.Tensor,
               pixels: torch.Tensor = None,
               ray_idx: torch.Tensor = None, depth_range: Any = None, iter: int = None, mode: int = None) -> Dict[
        str, Any]:
        ...
        Args:
            ...
            depth_range (list or torch.Tensor): Min and max value for the depth sampling along the ray.
                                The same for all rays here. If torch.Tensor, suggesting a depthmap guided sampling
            iter (int): iteration
        ...
        if isinstance(depth_range, torch.Tensor):
            assert len(depth_range) == batch_size

        if pixels is not None:
            center, ray = camera.get_center_and_ray_at_pixels(pose, pixels, intr=intr)
            # [B, N_rays, 3] and [B, N_rays, 3]
            if isinstance(depth_range, torch.Tensor):
                # Depth Guidede Sampling, xy_grid
                depth_range = einops.rearrange(depth_range, 'b h w -> b h w 1')
                depth_range = depth_range[:, pixels[:, 1].long(), pixels[:, 0].long(), :]
        else:
            # from ray_idx
            ...
            if ray_idx is not None:
                if len(ray_idx.shape) == 2 and ray_idx.shape[0] == batch_size:
                    ...
                    batch_idx_flattened = torch.arange(start=0, end=batch_size).unsqueeze(-1).repeat(1,
                                                                                                     n_rays).long().view(
                        -1)
                    ray_idx_flattened = ray_idx.reshape(-1).long()

                    # consider only subset of rays
                    center, ray = center[batch_idx_flattened, ray_idx_flattened], ray[
                        batch_idx_flattened, ray_idx_flattened]  # [B*N_rays, 3]
            ...
            if isinstance(depth_range, torch.Tensor):
                # Depth Guidede Sampling
                depth_range = einops.rearrange(depth_range, 'b h w -> b (h w) 1')
                depth_range = depth_range[:, ray_idx]
        ...
        pred_coarse = self.nerf.forward_samples(opt, center, ray, depth_samples, intr, H, W, depth_range,
    ...
    def render_by_slices(self, opt: Dict[str, Any], pose: torch.Tensor, H: int, W: int,
        ...
        if opt.nerf.fine_sampling:
            if (hasattr(opt.nerf,
                        'ratio_start_fine_sampling_at_x') and opt.nerf.ratio_start_fine_sampling_at_x is not None \
                    and iter is not None and iter < opt.max_iter * opt.nerf.ratio_start_fine_sampling_at_x):
        ...
        for c in range(0, H * W, opt.nerf.rand_rays):
            ...
            ret = self.render(opt, pose, H=H, W=W, intr=intr, ray_idx=ray_idx, depth_range=depth_range, iter=iter,
    ...
    def sample_depth(self, opt: Dict[str, Any], batch_size: int, n_samples: int, H: int, W: int, depth_range: Any, \
        ...
        elif isinstance(depth_range, torch.Tensor):
            depth_range = depth_range.unsqueeze(2)
            depth_min, depth_max = depth_range * (1 - opt.nerf.depth.relrange), depth_range * (
                        1 + opt.nerf.depth.relrange)
        else:
            raise TypeError()
        ...
        rand_samples += torch.arange(n_samples, device=self.device)[None, None, :,
                        None].float()  # the added part is [1, 1, N, 1] ==> [B,HW,N,1]
        ...
        depth_samples = dict(
            ...
            monoguided=depth_samples,
            datainverse=1 / (depth_samples + 1e-8)
    ...
    def sample_depth_diff_max_range_per_ray(self, opt: Dict[str, Any], batch_size: int, n_samples: int, H: int, W: int,
        ...
        # depth_max is (B, HW)
        depth_samples = rand_samples / n_samples * (
                    depth_max.unsqueeze(-1).unsqueeze(-1) - depth_min) + depth_min  # [B,HW,N,1]
