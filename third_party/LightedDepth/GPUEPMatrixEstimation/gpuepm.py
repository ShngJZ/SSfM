...
def gpuepm_function(kpts0, kpts1, ransac_iter=5, ransac_threshold=0.0001, num_test_chirality=10):
    '''
    ...
    Ems, Pms, inliers_num, inliers_indicator = GPUEPM.gpu_ep_ransac(
        kpts0,
        kpts1,
        num_test_chirality,
        ransac_test_points,
    ...
    _, idx_sorted = torch.sort(-inliers_num)
    Em, Pm, inliers = Ems[idx_sorted[0]], Pms[idx_sorted[0]], inliers_indicator[idx_sorted[0]]
    R, t, inliers = Pm[0:3, 0:3].cpu().numpy(), Pm[0:3, 3:4].cpu().numpy(), inliers.cpu().numpy()

    return R, t, inliers

def gpuepm_function_topk(kpts0, kpts1, ransac_iter=5, ransac_threshold=0.0001, num_test_chirality=10, topk=20):
    '''
    Function follow the convention of OpenCV - cv2.findEssentialMat
    ...
    _, idx_sorted = torch.sort(-inliers_num)
    return Pms[idx_sorted[0:topk]].float()
